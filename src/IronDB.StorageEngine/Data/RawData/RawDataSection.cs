...
public unsafe class RawDataSection
{
    ...
    public List<long> GetAllIdsInSectionContaining(long id)
    {
        if (Contains(id) == false)
        {
            ...
            {
                EngineUnrecoverableErrorException.Raise(_llt,
    ...
    public bool TryWriteDirect(long id, int size, bool compressed, out byte* writePos)
    {
        ...
        if (posInPage >= pageHeader->NextAllocation)
            EngineUnrecoverableErrorException.Raise(_llt, $"Asked to load a past the allocated values: {id} from page {pageHeader->PageNumber}");

        var sizes = (RawDataEntrySizes*)((byte*)pageHeader + posInPage);
        if (sizes->IsFreed)
            EngineUnrecoverableErrorException.Raise(_llt, $"Asked to load a value that was already freed: {id} from page {pageHeader->PageNumber}");
    ...
    public static RawDataEntrySizes* GetRawDataEntrySizeFor(LowLevelTransaction tx, long id)
    {
        ...
        {
            if (posInPage == 0)
                EngineUnrecoverableErrorException.Raise(tx,
    ...
    public RawDataSection Free(long id)
    {
        ...
        if (Contains(id) == false)
        {
            ...
            if (actualSection.Contains(id) == false)
                EngineUnrecoverableErrorException.Raise(_llt, $"Cannot delete {id} because the raw data section starting in {sectionPageNumber} with size {actualSection.AllocatedSize} doesn't own it. Possible data corruption?");
        ...
        var sizes = (RawDataEntrySizes*)((byte*)pageHeader + posInPage);
        if (sizes->IsFreed)
            EngineUnrecoverableErrorException.Raise(_llt, $"Asked to free a value that was already freed: {id} from page {pageHeader->PageNumber}");
