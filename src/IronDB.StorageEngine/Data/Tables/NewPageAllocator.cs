...
namespace IronDB.StorageEngine.Data.Tables;
...
public sealed class NewPageAllocator
{
        ...
        using (var it = fst.Iterate())
        {
            ...
            do
            {
                using (it.Value(out Slice slice))
                {
                    ...
                    for (int i = 0; i < _numberOfPagesToAllocate; i++)
                    {
                        if (!PtrBitVector.GetBitInPointer(ptr, i))
                        {
    ...
    private struct SectionsIterator : IEnumerator<long>
    {
        ...
        {
            if (_index >= NumberOfPagesInSection)
            {
                _isDone = _iterator.MoveNext() == false;
                if (_isDone)
                    return false;

                _index = 0;
                Current = _iterator.CurrentKey;
            }
            else
            {
                Current = _index + _iterator.CurrentKey;
                _index++;
            }
            return true;
        }
