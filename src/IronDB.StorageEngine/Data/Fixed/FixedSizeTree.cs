...
using IronDB.StorageEngine.Debugging;
    ...
    where TVal : unmanaged, IBinaryNumber<TVal>, IMinMaxValue<TVal>
{
    ...
    public FixedSizeTree(
        LowLevelTransaction tx, 
        Tree parent, 
        Slice treeName, 
        ushort valSize, 
        bool clone = true,
        bool isIndexTree = false, 
        NewPageAllocator? newPageAllocator = null)
    {
        _tx = tx;
        _parent = parent;
    ...
    {
        throw new InvalidFixedSizeTree(Type?.ToString() ?? string.Empty);
    }
    ...
    private FixedSizeTreePage<TVal>? PageSplit(FixedSizeTreePage<TVal> page, TVal key)
    {
        ...
        if (parentPage == null) // root split
        {
            ...
            parentPage.StartPosition = Constants.FixedSizeTree.PageHeaderSize;
        ...
        if (page.IsLeaf) // simple case of splitting a leaf pageNum
        {
            ...
            newPage.StartPosition = Constants.FixedSizeTree.PageHeaderSize;
    ...
    private static void AddLeafKey(FixedSizeTreePage<TVal> page, int position, TVal key)
    {
    ...
    private byte* AddEmbeddedEntry(TVal key, out bool isNew)
    {
        using (_tx.Allocator.Allocate(Constants.Storage.PageSize, out ByteString tmp))
        {
            ...
            using (_parent.DirectAdd(_treeName, sizeof(FixedSizeTreeHeader.Embedded) + newSize, out byte* newData))
            {
    ...
    private long DeleteRangeEmbedded(TVal start, TVal end)
    {
        ...
        using (_tx.Allocator.Allocate(Constants.Storage.PageSize, out ByteString tmp))
        {
            ...
            using (_parent.DirectAdd(_treeName, newDataSize, out byte* newData))
            {
