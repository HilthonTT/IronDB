...
{
    private static readonly byte[] Context = Encodings.Utf8.GetBytes("IronDB!");
    ...
    public CryptoPager(AbstractPager inner) 
        : base(inner.Options, inner.Logger, inner.UsePageProtection)
    {
        if (!inner.Options.Encryption.IsEnabled)
        {
    ...
    public override byte* AcquirePagePointerForNewPage(
    ...
    public override T AcquirePagePointerHeader<T>(
        IPagerLevelTransactionState? tx,
        long pageNumber, 
        PagerState? pagerState = null)
    {
    ...
    {
        if (tx.CryptoPagerTransactionState is null)
        {
            return;
        }

        if (!tx.CryptoPagerTransactionState.TryGetValue(this, out CryptoTransactionState? state))
        {
            return;
        }

        if (!state.TryGetValue(page, out var buffer))
        {
            return;
        }
    ...
    {
        if (buffer?.OriginalSize is not null && buffer.OriginalSize != 0)
        {
