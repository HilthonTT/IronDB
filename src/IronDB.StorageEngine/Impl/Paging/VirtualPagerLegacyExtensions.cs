...
public static unsafe class VirtualPagerLegacyExtensions
{
    ...
    public static byte* AcquirePagePointerWithOverflowHandling<T>(
        this AbstractPager pager, 
        T? tx, 
        long pageNumber, 
        PagerState? pagerState) 
        where T : IPagerLevelTransactionState
    {
        ...
        where T : IPagerLevelTransactionState
    {
        ...
        // Case 2: Page is overflow and already mapped large enough ==> no problem, returning a pointer to existing mapping
        if (!pager.EnsureMapped(tx, pageNumber, GetNumberOfOverflowPages(pageHeader->OverflowSize)))
        {
            return (byte*)pageHeader;
        }
