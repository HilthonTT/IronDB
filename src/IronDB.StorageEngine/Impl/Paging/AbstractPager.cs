...
{
    public readonly IronLogger Logger;
    private readonly StorageEnvironmentOptions _options = default!;

    public readonly static ConcurrentDictionary<string, uint> PhysicalDrivePerMountCache = new();

    protected static readonly object WorkingSetIncreaseLocker = new();
    ...
    private readonly Lock _pagerStateModificationLocker = new();
    ...
    protected AbstractPager(IronLogger logger)
    {
        Logger = logger;
    ...
    protected AbstractPager(
        ...
        bool usePageProtection = false) : this(logger)
    {
        DisposeOnceRunner = new DisposeOnce<SingleAttempt>(() =>
        {
            if (FileName?.FullPath is not null)
            {
    ...
    {
        if (DisposeOnceRunner?.Disposed == true)
        {
        ...
        lock (_pagerStateModificationLocker)
        {
            ...
            if (ShouldLockMemoryAtPagerLevel())
            {
                ...
                try
                {
                    ...
                    {
                        if (info.Size == 0 || info.BaseAddress is null)
                        {
                            continue;
                        }
    ...
    protected void Lock(byte* address, long sizeToLock, Data.Containers.Container.TransactionState? state)
    {
...
#pragma warning restore CA1416 // Validate platform compatibility
        ...
        sb.Append($"Unable to lock memory for {FileName} with size {new Size(sizeToLock, SizeUnit.Bytes)}), with encrypted databases we lock some memory in order to avoid leaking secrets to disk. Treating this as a catastrophic error and aborting the current operation.{Environment.NewLine}");

        if (PlatformDetails.RunningOnPosix)
        {
            sb.Append($"The admin may configure higher limits using: 'sudo prlimit --pid {currentProcess.Id} --memlock={sizeToLock}' to increase the limit. (It's recommended to do that as part of the startup script){Environment.NewLine}");
        }
        else
        {
            sb.Append($"Already tried to raise the the process min working set to {new Size(currentProcess.MinWorkingSet.ToInt64(), SizeUnit.Bytes)} but still got a failure.{Environment.NewLine}");
        }

        sb.Append("This behavior is controlled by the 'Security.DoNotConsiderMemoryLockFailureAsCatastrophicError' setting (expert only, modifications of this setting is not recommended).");
    ...
    public virtual byte* AcquirePagePointerForNewPage(
    ...
    public virtual T AcquirePagePointerHeader<T>(IPagerLevelTransactionState? tx, long pageNumber, PagerState? pagerState = null)
        where T : unmanaged
    {
    ...
    public virtual void BreakLargeAllocationToSeparatePages(
        IPagerLevelTransactionState? tx, 
        long valuePositionInScratchBuffer, 
        long actualNumberOfAllocatedScratchPages)
    {
    ...
    public bool Disposed => DisposeOnceRunner?.Disposed == true;
    ...
    {
        // this is a separate method because we don't want to have an exception throwing in the hot path
        EngineUnrecoverableErrorException.Raise(_options,
    ...
    {
        if (!PlatformDetails.CanPrefetch || !CanPrefetchAhead)
        {
    ...
    private struct PageIterator : IEnumerator<long>
    {
        ...
        public PageIterator(long pageNumber, int pagesToPrefetch)
        {
            ...
            _endPage = pageNumber + pagesToPrefetch;
    ...
    public void MaybePrefetchMemory<T>(T pagesToPrefetch) where T : struct, IEnumerator<long>
    {
        ...
        do
        {
            ...
            if (_pagerState is not null && _pagerState.ShouldPrefetchSegment(pageNumber, out void* virtualAddress, out long bytes))
            {
        ...
        _pagerState?.CheckResetPrefetchTable();
    }

    public virtual void DiscardPages(long pageNumber, int numberOfPages)
    {
        if (!_options.DiscardVirtualMemory)
        {
            return;
        }
