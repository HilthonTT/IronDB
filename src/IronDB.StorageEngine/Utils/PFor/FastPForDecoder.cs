...
using IronDB.StorageEngine.Utils.PFor;
...
public unsafe struct FastPForDecoder : IDisposable
{
    ...
    public readonly bool IsValid => _input is not null;
    ...
    private readonly int NumberOfIntegersInBuffer => _buffer.Length / sizeof(uint);
    ...
    {
        ArgumentOutOfRangeException.ThrowIfLessThanOrEqual(size, sizeof(PForHeader));
    ...
    public int Read(long* output, int outputCount)
    {
        ...
        int read = 0;
        Span<long> outputSpan = new(output, outputCount);
