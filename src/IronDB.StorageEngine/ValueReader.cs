...
public unsafe struct ValueReader(byte* val, int len)
{
    ...
    private readonly byte[] EnsureTempBuffer(int size)
    {
    ...
    public readonly int CompareTo(ValueReader other)
    {
    ...
    public readonly UnmanagedSpan<T> ToUnmanagedSpan<T>() where T : unmanaged
    {
        PortableExceptions.ThrowIf<InvalidOperationException>(
            _len % Unsafe.SizeOf<T>() != 0, $"The length of the value is not a multiple of the size of the type {nameof(T)}");
