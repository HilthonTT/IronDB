...
public unsafe struct Slice
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static implicit operator ReadOnlySpan<byte>(Slice x) => new(x.Content.Ptr, x.Content.Length);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static explicit operator Span<byte>(Slice x) => new(x.Content.Ptr, x.Content.Length);
    ...
    public readonly byte this[int index]
    {
        ...
        get
        {
            Debug.Assert(Content.Ptr is not null, "Uninitialized slice!");
    ...
    public readonly Slice Skip(ByteStringContext context, int bytesToSkip, ByteStringType type = ByteStringType.Mutable)
    {
    ...
    {
        Content.CopyTo(from, dest, offset, count);
    }
    ...
    {
        return Content.ToString(Encodings.Utf8);
    }
    ...
    public readonly Span<byte> AsSpan()
    {
        ...
        return Size == 0 ? [] : new Span<byte>(Content.Ptr, Size);
    }
    ...
    public readonly ReadOnlySpan<byte> AsReadOnlySpan()
    {
        // From the point of view of the Slice, if the content is null the size is 0 or the empty slice. Therefore,
        // requesting a span of an empty slice is valid to just return `Span<byte>.Empty`.
        return Size == 0 ? [] : new ReadOnlySpan<byte>(Content.Ptr, Content.Length);
    }
}
