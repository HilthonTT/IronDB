...
internal sealed unsafe class PagerState
{
    ...
    {
        public MemoryMappedFile MappedFile { get; set; } = default!;
    ...
    public PagerState(AbstractPager pager, long prefetchSegmentSize, long prefetchResetThreshold, AllocationInfo? allocationInfo = null)
    {
        ...
        if (allocationInfo != null)
        {
            ...
            File = allocationInfo.MappedFile;
...
#pragma warning disable CS0649 // set by the pager once tx-copy support is implemented
...
#pragma warning restore CS0649
    ...
    {
        if (AllocationInfos is not null)
        {
    ...
    {
        long segmentNumber = (pageNumber * Constants.Storage.PageSize) >> _segmentShift;
        ...
        if (segmentState < AlreadyPrefetch)
        {
            ...
            if (segmentState == AlreadyPrefetch)
            {
                ...
                virtualAddress = MapBase + segmentNumber * _prefetchSegmentSize;
                return true;
            }
        }
    ...
    public void CheckResetPrefetchTable()
    {
        if (_refreshCounter > _prefetchResetThreshold)
        {
            ...
            Array.Clear(_prefetchTable, 0, _prefetchTable.Length);
        }
    }
    ...
    public void CopyPrefetchState(PagerState olderInstance)
    {
        ...
        Array.Copy(olderInstance._prefetchTable, _prefetchTable, olderInstance._prefetchTable.Length);
    }

    public void DiscardDataOnDisk()
    {
        if (!_pager.Options.DiscardVirtualMemory)
        {
            return;
        }
