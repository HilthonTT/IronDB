...
using static IronDB.Core.Server.Platform.PalDefinitions;
using static IronDB.Core.Server.Platform.Pal;

namespace IronDB.StorageEngine;

public sealed class GlobalPrefetchingBehavior
{
    private const string PrefetchingThreadName = "Engine Prefetching Thread";

    private readonly IronLogger _log = IronLogManager.Instance.GetLoggerForGlobalEngine<GlobalPrefetchingBehavior>();

    public readonly BlockingCollection<PrefetchRanges> CommandQueue = new(128);

    internal static readonly Lazy<GlobalPrefetchingBehavior> GlobalPrefetcher = new(() =>
    {
        var prefetcher = new GlobalPrefetchingBehavior();
        var thread = new Thread(prefetcher.EnginePrefetcher)
        {
            IsBackground = true,
    ...
    private unsafe void EnginePrefetcher()
    {
