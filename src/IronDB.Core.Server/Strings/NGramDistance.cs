...
public struct NGramDistance : IStringDistance
{
    ...
    public readonly float GetDistance(ReadOnlySpan<byte> input, ReadOnlySpan<byte> other)
    {
        // TODO PERF: Get rid of this conversion unless strictly necessary to do so and allocations.
        string source = Encoding.UTF8.GetString(input.Slice(0));
        string target = Encoding.UTF8.GetString(other[..^1]);
