...
#if MEM_GUARD_STACK
    : Core.Debugging.DebugStuff.IElectricFencedMemory
#endif
{
    public static ElectricFencedMemory Instance = new();
#if MEM_GUARD_STACK
        public ConcurrentDictionary<IntPtr, Tuple<int, string>> Allocs = new();

        public ConcurrentDictionary<nint, ConcurrentSet<string>> DoubleMemoryReleases = new();

        public ConcurrentDictionary<JsonOperationContext, string> ContextAllocations = new();
