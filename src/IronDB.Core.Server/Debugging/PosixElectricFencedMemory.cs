...
internal static unsafe class PosixElectricFencedMemory
{
    ...
    public static byte* Allocate(int size)
    {
        ...
        var msync = Syscall.msync((IntPtr)virtualAlloc, (uint)allocatedSize, MsyncFlags.MS_SYNC | MsyncFlags.MS_INVALIDATE);
        if (msync != 0)
        {
        ...
        msync = Syscall.msync((IntPtr)virtualAlloc, (uint)allocatedSize, MsyncFlags.MS_SYNC | MsyncFlags.MS_INVALIDATE);
        if (msync != 0)
        {
            throw new InvalidOperationException("Failed to free call msync " + (IntPtr)virtualAlloc + ". Err=" + Marshal.GetLastWin32Error());
        }
    ...
    public static void Free(byte* p)
    {
        ...
        // }
        Syscall.munmap((IntPtr)address, (uint)dwSize);
