...
using IronDB.Core.Server.Utils.DiskStatsGetter;
...
{
    private static readonly IronLogger Logger = IronLogManager.Instance.GetLoggerForIronServer(typeof(DiskUtils));
    ...
    {
        if (string.IsNullOrWhiteSpace(path) || PlatformDetails.RunningOnPosix)
        {
            return null;
        }
    ...
    private static unsafe string? GetWindowsRealPathByHandle(IntPtr handle)
    {
        ...
        if (!GetPath(256, out var realPath))
        {
            if (!GetPath((uint)WindowsMaxPath, out realPath))
            {
                return null;
            }
        }
