...
internal sealed class LinuxDiskStatsGetter(TimeSpan minInterval) : DiskStatsGetter<LinuxDiskStatsRawResult>(minInterval)
{
    private static readonly IronLogger Logger = IronLogManager.Instance.GetLoggerForIronServer<LinuxDiskStatsGetter>();

    protected override DiskStatsResult CalculateStats(LinuxDiskStatsRawResult currentInfo, State state)
    {
        double diff = (currentInfo.Time - state.Result.RawSampling.Time).TotalSeconds;
        var diskSpaceResult = new DiskStatsResult
        {
            ...
            ReadThroughput = 
                new((long)((currentInfo.ReadSectors - state.Result.RawSampling.ReadSectors) * 512 / diff), SizeUnit.Bytes),

            WriteThroughput = 
                new((long)((currentInfo.WriteSectors - state.Result.RawSampling.WriteSectors) * 512 / diff), SizeUnit.Bytes),
    ...
    {
        int valuesIndex = 0;
        while (!content.IsEmpty && valuesIndex < values.Length)
        {
            var position = 0;
            ...
            if (!Utf8Parser.TryParse(content, out long v, out var consumed))
            {
                if (content[0] == '\n')
                {
                    break;
