...
internal unsafe partial struct Avx2VectorizedSort
{
    ...
    internal struct DoubleConfig
    {
        ...
        public const int PartitionTempSizeInBytes = PartitionTempSizeInElements * sizeof(double);
        public const int ElementAlign = sizeof(double) - 1;
    }


    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static double* align_left_scalar_uncommon(double* read_left, double pivot, ref double* tmp_left, ref double* tmp_right)
    {
        ...
        if (((ulong)read_left & DoubleConfig.ALIGN_MASK) == 0)
            return read_left;

        var next_align = (double*)(((ulong)read_left + DoubleConfig.ALIGN) & ~DoubleConfig.ALIGN_MASK);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static double* align_right_scalar_uncommon(double* readRight, double pivot, ref double* tmpLeft, ref double* tmpRight)
    {
        ...
        if (((ulong)readRight & DoubleConfig.ALIGN_MASK) == 0)
            return readRight;

        var nextAlign = (double*)((ulong)readRight & ~DoubleConfig.ALIGN_MASK);
    ...
    private static void align_vectorized(
        ...
        in V p,
        byte* pBase,
        ref double* readLeft, ref double* readRight,
        ref double* tmpStartLeft, ref double* tmpLeft,
        ref double* tmpStartRight, ref double* tmpRight)
    { throw new NotImplementedException(); }

    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private static void partition_block(V dataVec, V p, byte* pBase, ref double* left, ref double* right) { throw new NotImplementedException(); }


    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private static void LoadAndPartition1Vectors(double* dataPtr, V P, byte* pBase, ref double* writeLeftPtr, ref double* writeRightPtr)
    {
        ...
        var d1 = LoadAlignedVector256((double*)(dataPtr + N * 0));
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private static void LoadAndPartition4Vectors(double* dataPtr, V P, byte* pBase, ref double* writeLeftPtr, ref double* writeRightPtr)
    {
        // PERF: Unroll and CompressWrite support is been treated as a constant because we make sure the caller
        //       treats that parameter already as a constant @ JIT time causing a cascade.
        ...
        var d4 = LoadAlignedVector256((double*)(dataPtr + N * 0));
        var d3 = LoadAlignedVector256((double*)(dataPtr + N * 1));
        var d2 = LoadAlignedVector256((double*)(dataPtr + N * 2));
        var d1 = LoadAlignedVector256((double*)(dataPtr + N * 3));
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    private static void LoadAndPartition8Vectors(double* dataPtr, V P, byte* pBase, ref double* writeLeftPtr, ref double* writeRightPtr)
    {
        // PERF: Unroll and CompressWrite support is been treated as a constant because we make sure the caller
        //       treats that parameter already as a constant @ JIT time causing a cascade.
        ...
        var d8 = LoadAlignedVector256((double*)(dataPtr + N * 0));
        var d7 = LoadAlignedVector256((double*)(dataPtr + N * 1));
        var d6 = LoadAlignedVector256((double*)(dataPtr + N * 2));
        var d5 = LoadAlignedVector256((double*)(dataPtr + N * 3));
        var d4 = LoadAlignedVector256((double*)(dataPtr + N * 4));
        var d3 = LoadAlignedVector256((double*)(dataPtr + N * 5));
        var d2 = LoadAlignedVector256((double*)(dataPtr + N * 6));
        var d1 = LoadAlignedVector256((double*)(dataPtr + N * 7));
    ...
    {
        Debug.Assert(right - left >= DoubleConfig.SmallSortThresholdElements);
        Debug.Assert(((long)left & DoubleConfig.ElementAlign) == 0);
        Debug.Assert(((long)right & DoubleConfig.ElementAlign) == 0);
        ...
        *right = double.MaxValue;
        ...
        var tmpStartLeft = (double*)_tempPtr;
        var tmpLeft = tmpStartLeft;
        var tmpStartRight = tmpStartLeft + DoubleConfig.PartitionTempSizeInElements;
        var tmpRight = tmpStartRight;

        tmpRight -= DoubleConfig.N;
        ...
        Debug.Assert(((ulong)readLeft & DoubleConfig.ALIGN_MASK) == 0);
        Debug.Assert(((ulong)readRight & DoubleConfig.ALIGN_MASK) == 0);

        Debug.Assert((((ulong)readRight - (ulong)readLeft) % DoubleConfig.ALIGN) == 0);
        Debug.Assert((readRight - readLeft) >= DoubleConfig.Unroll * 2);
        ...
        var pointers = stackalloc double*[2];
        pointers[0] = readLeftV;
        pointers[1] = readRightV - DoubleConfig.Unroll * DoubleConfig.N;
        ...
        readLeftV += DoubleConfig.N * DoubleConfig.Unroll;
        readRightV -= DoubleConfig.N * DoubleConfig.Unroll * 2;

        double* nextPtr;

        var writeLeft = left;
        var writeRight = right - DoubleConfig.N;

        while (readLeftV < readRightV)
        {
            if ((byte*)writeRight - (byte*)readRightV < (2 * (DoubleConfig.Unroll * DoubleConfig.N) - DoubleConfig.N) * sizeof(double))
            {
                ...
                nextPtr = readRightV;
                readRightV -= DoubleConfig.N * DoubleConfig.Unroll;
            }
            else
            {
        ...
        int unrollHalf = DoubleConfig.Unroll / 2;
        readRightV += DoubleConfig.N * unrollHalf;
        while (readLeftV < readRightV)
        {
            if ((byte*)writeRight - (byte*)readRightV < (2 * (unrollHalf * DoubleConfig.N) - DoubleConfig.N) * sizeof(double))
            {
            ...
            else
            {
                ...
                // PERF: Ensure that JIT never emits cmov here.
                nextPtr = readLeftV;
                readLeftV += DoubleConfig.N * unrollHalf;
            }

            LoadAndPartition4Vectors(nextPtr, P, pBase, ref writeLeft, ref writeRight);
        }

        readRightV += DoubleConfig.N * (unrollHalf - 1);
        ...
        while (readLeftV <= readRightV)
        {
            if ((byte*)writeRight - (byte*)readRightV < DoubleConfig.N * sizeof(double))
            {
                nextPtr = readRightV;
                readRightV -= DoubleConfig.N;
            }
            else
            {
                // PERF: Ensure that JIT never emits cmov here.
                nextPtr = readLeftV;
                readLeftV += DoubleConfig.N;
            }
        ...
        Unsafe.CopyBlockUnaligned(writeLeft, tmpStartLeft, (uint)(leftTmpSize * sizeof(double)));
        writeLeft += leftTmpSize;


        var rightTmpSize = tmpStartRight - tmpRight;
        Unsafe.CopyBlockUnaligned(writeLeft, tmpRight, (uint)(rightTmpSize * sizeof(double)));
    ...
    private static unsafe void SwapIfGreater(double* leftPtr, double* rightPtr)
    {
        if (*leftPtr <= *rightPtr) return;
        Swap(leftPtr, rightPtr);
    }

    private static unsafe void SwapIfGreater3(in double* leftPtr, in double* middlePtr, in double* rightPtr)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static void down_heap(long i, long n, double* lo)
    {
        var d = *(lo + i - 1);
        long child;
        while (i <= n / 2)
    ...
    static void heap_sort(double* lo, double* hi)
    {
        long n = hi - lo + 1;
    ...
    internal void sort(double* left, double* right, double left_hint, double right_hint, long hint, int depth_limit)
    {
        ...
        if (length <= DoubleConfig.SmallSortThresholdElements)
        {
        ...
        var preAlignedLeft = (double*)((ulong)left & ~DoubleConfig.ALIGN_MASK);
        var cannotPreAlignLeft = ((long)preAlignedLeft - (long)_startPtr) >> 63;
        var preAlignLeftOffset = (preAlignedLeft - left) + (DoubleConfig.N & cannotPreAlignLeft);
        if ((hint & DoubleConfig.REALIGN_LEFT) != 0)
        {
            // Alignment flow:
        ...
        var preAlignedRight = (double*)(((ulong)right - 1 & ~DoubleConfig.ALIGN_MASK) + DoubleConfig.ALIGN);
        var cannotPreAlignRight = ((long)_endPtr - (long)preAlignedRight) >> 63;
        var preAlignRightOffset = (preAlignedRight - right - (DoubleConfig.N & cannotPreAlignRight));
        if ((hint & DoubleConfig.REALIGN_RIGHT) != 0)
        {
        ...
        Debug.Assert(((ulong)(left + (hint & 0xFFFFFFFF)) & DoubleConfig.ALIGN_MASK) == 0);
        Debug.Assert(((ulong)(right + (hint >> 32)) & DoubleConfig.ALIGN_MASK) == 0);
        ...
        sort(left, sep - 2, left_hint, *sep, hint | DoubleConfig.REALIGN_RIGHT, depth_limit);

        sort(sep, right, *(sep - 2), right_hint, hint | DoubleConfig.REALIGN_LEFT, depth_limit);
