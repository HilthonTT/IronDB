...
using IronDB.Core.Server.LowMemory;
...
public static unsafe class PlatformSpecific
{
    ...
    {
        public static byte* Allocate4KbAlignedMemory(long size, out Core.Utils.NativeMemory.ThreadStats? thread)
        {
            Debug.Assert(size >= 0);

            thread = Core.Utils.NativeMemory.ThreadAllocations.Value;
            ...
            var allocate4KbAlignedMemory = Win32MemoryProtectMethods.VirtualAlloc(
                null, (UIntPtr)size, 
                Win32MemoryProtectMethods.AllocationType.COMMIT,
