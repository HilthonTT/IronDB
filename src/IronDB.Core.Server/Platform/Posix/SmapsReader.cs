...
internal sealed class SmapsReader : ISmapsReader
{
    ...
    private readonly SmapsReaderResults _smapsReaderResults = new();
    ...
    public SmapsReadResult<T> CalculateMemUsageFromSmaps<T>() 
        where T : struct, ISmapsReaderResultAction
    {
        using var currentProcess = Process.GetCurrentProcess();
        using var fileStream = new FileStream(GetSmapsPath(currentProcess.Id), FileMode.Open, FileAccess.Read, FileShare.Read);
        return CalculateMemUsageFromSmaps<T>(fileStream, currentProcess.Id);
    }

    public SmapsReadResult<T> CalculateMemUsageFromSmaps<T>(Stream fileStream, int pid) 
        where T : struct, ISmapsReaderResultAction
    {
        _endOfBuffer[0] = 0;
        _endOfBuffer[1] = 0;
        ...
        while (true)
        {
            ...
            for (var i = offsetForNextBuffer; i < _endOfBuffer[_currentBuffer]; i++)
            {
                ...
                else if (term == _lockedBytes)
                {
                    ...
                    if (!resultString.EndsWith(".engine") &&
                        !resultString.EndsWith(".buffers"))
                    {
                        continue;
                    }
    ...
    private static string GetSmapsPath(int pid)
    {
    ...
    [DoesNotReturn]
    private static void ThrowNotContainsValidValue(byte[] term, int processId)
    {
        throw new InvalidDataException($"Found '{Encoding.UTF8.GetString(term)}' string in /proc/{processId}/smaps, but no value");
    }

    [DoesNotReturn]
    private static void ThrowNotContainsKbValue(byte[] term, int processId, string addtionalInfo)
    {
        throw new InvalidDataException(
