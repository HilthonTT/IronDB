...
using IronDB.Core.Server.Exceptions;
...
public static unsafe class Syscall
{
    ...
    public static void ThrowLastError(int lastError, string? msg = null)
    {
        ...
        throw error switch
        {
            Errno.ENOMEM => new OutOfMemoryException($"ENOMEM on {msg}"),
            Errno.ENOENT => new FileNotFoundException($"ENOENT on {msg}"),
            Errno.ENOSPC => new DiskFullException($"ENOSPC on {msg}"),
            _ => new InvalidOperationException(error + " " + msg),
        };
    }
        ...
        foreach (var m in allMounts)
        {
            ...
            if (filepath.StartsWith(m.Name))
            {
                if (mountNameSize > matchSize)
                {
                    ...
                    syncAllowed = m.DriveFormat switch
                    {
                        "cifs" or "nfs" => false,
                        _ => true,
                    };
