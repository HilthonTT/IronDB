...
internal sealed class Encoder3GramSymbolSelector<TSampleEnumerator>
      where TSampleEnumerator : IReadOnlySpanEnumerator
{
    private struct ByFrequencyComparer : IComparer<SymbolFrequency>
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public readonly int Compare(SymbolFrequency x, SymbolFrequency y)
        {
    ...
    public FastList<SymbolFrequency> SelectSymbols(
        TSampleEnumerator keys, 
        int dictionarySize, 
        FastList<SymbolFrequency>? symbolFrequenciesList = null)
    {
        ...
        if (symbolFrequenciesList is null)
        {
            symbolFrequenciesList = [];
        }
        else
        {
        ...
        for (int i = 0; i < _intervalBoundaries.Count; i++)
        {
    ...
    private void CountIntervalFreq(TSampleEnumerator keys, FastList<int> intervalFrequencies, FastList<Symbol> intervalPrefixes, FastList<Symbol> intervalBoundaries)
    {
        ...
        while (keys.MoveNext(out var key))
        {
            ...
            while (pos < key.Length)
            {
                ...
                var keyAsInteger = (key.Length - pos) switch
                {
                    1 => (currentKey << 16),
                    2 => (currentKey << 16 | Unsafe.AddByteOffset(ref currentKey, 1) << 8),
                    _ => (currentKey << 16 | Unsafe.AddByteOffset(ref currentKey, 1) << 8 | Unsafe.AddByteOffset(ref currentKey, 2)),
                };
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int BinarySearch(int key, FastList<Symbol> intervalBoundaries)
    {
        int l = 0;
    ...
    private void FillInGap(
        FastList<SymbolFrequency> mostFrequentSymbols, 
        FastList<Symbol> intervalPrefixes, 
        FastList<Symbol> intervalBoundaries)
    {
