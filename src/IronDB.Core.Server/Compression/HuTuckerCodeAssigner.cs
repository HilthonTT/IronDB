...
internal sealed class HuTuckerCodeAssigner
{
    internal sealed class Node
    {
        ...
        public bool IsLeaf => LeftChild is null;
    }

    private readonly FastList<int> _cachedNodeIndexList = [];

    // For now we allocate, we don't need to do so. 
    private readonly FastList<SymbolFrequency> _symbolsList = [];
    private readonly FastList<int> _codeLengthList = [];
    private readonly FastList<Node?> _nodesList = [];
    private Node? _root;

    public FastList<SymbolCode> AssignCodes(
        FastList<SymbolFrequency> frequency, 
        FastList<SymbolCode>? symbolCodeList = null)
    {
        Clear();
    ...
    private void BuildBinaryTree()
    {
        ...
        for (int len = maxCodeLen; len > 0; len--)
        {
            ...
            for (int i = 0; i < nodeIndexList.Count; i += 2)
            {
                ...
                {
                    Node newNode = new(leftNode.LeftIdx, rightNode.RightIdx, leftNode, rightNode);
