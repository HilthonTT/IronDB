    ...
    where T : unmanaged
{
    ...
    public UnmanagedSpan<T> Slice(int offset, int length)
    {
        return new UnmanagedSpan<T>(Address + offset, length);
    }
    ...
    public static UnmanagedSpan<T> operator +(UnmanagedSpan<T> pointer, int offset) =>
        new(pointer.Address + offset, pointer.Length - offset);
