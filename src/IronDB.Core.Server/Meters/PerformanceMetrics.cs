...
public abstract class PerformanceMetrics
{
    ...
    protected void AddHistoryStats(IEnumerable<SummerizedItem> history, PerformanceMetricsStats stats)
    {
        foreach (var meter in history)
        {
            ...
            {
                Start = meter.TotalTimeStart.GetDefaultIronFormat(isUtc: true),
                End = meter.TotalTimeEnd.GetDefaultIronFormat(isUtc: true),
    ...
    public struct DurationMeasurement : IDisposable
    {
        ...
        public readonly void MarkInternalWindowStart() => _parent.MarkInternalWindowStart();

        public readonly void MarkInternalWindowEnd() => _parent.MarkInternalWindowEnd();

        public readonly void Dispose() => _parent.Mark(_counter, _commandsCounter, _start, DateTime.UtcNow);
    }
}
