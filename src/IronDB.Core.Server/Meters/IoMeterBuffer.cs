...
public sealed class IoMeterBuffer(int metricsBufferSize, int summaryBufferSize)
{
    ...
    private readonly MeterItem[] _buffer = new MeterItem[metricsBufferSize];
    private int _bufferPos = -1;

    private readonly SummerizedItem[] _summerizedBuffer = new SummerizedItem[summaryBufferSize];
    ...
    public void RemoveUntil(long ticks)
    {
        ...
        if (_summerizedPos != -1)
        {
            ...
            {
                if (_summerizedBuffer[pos] is null)
                {
                    continue;
                }
    ...
    internal void Mark(ref DurationMeasurement item)
    {
        ...
        if (Interlocked.CompareExchange(ref _buffer[adjustedTail], meterItem, null) is null)
        {
            return;
        }
