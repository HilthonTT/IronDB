...
internal sealed class TransactionPerformanceMetrics : PerformanceMetrics
{
    ...
    public sealed class InternalWindowDuration
    {
        ...
        public DynamicJsonValue ToJson()
        {
            ...
            {
                [nameof(Start)] = Start.GetDefaultIronFormat(isUtc: true),
                [nameof(End)] = End.GetDefaultIronFormat(isUtc: true),
    ...
    public TransactionPerformanceMetrics(
        int currentBufferSize, 
        int summaryBufferSize) : base(currentBufferSize, summaryBufferSize)
    {
    ...
    public override DynamicJsonValue ToJson()
    {
        ...
        foreach (var meter in recent)
        {
            ...
            {
                Start = meter.Start.GetDefaultIronFormat(isUtc: true),
    ...
    private IEnumerable<TransactionMeterItem> GetCurrentItems()
    {
        for (int pos = 0; pos < Buffer.Length; pos++)
        {
            if (Buffer[pos] is not TransactionMeterItem item)
            {
                continue;
            }
            yield return item;
    ...
    private List<TransactionMeterItem> GetRecentMetrics()
    {
