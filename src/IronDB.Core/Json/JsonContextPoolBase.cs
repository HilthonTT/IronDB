    ...
    where T : JsonOperationContext
{
    ...
    protected SharedMultipleUseFlag LowMemoryFlag = new();
    ...
    private readonly PerCoreContainer<T> _perCoreCache = new();
    private readonly CountingConcurrentStack<T> _globalStack = new();
    private readonly Timer _cleanupTimer;

    protected JsonContextPoolBase(IIronLogger logger)
    {
        _logger = logger;
    ...
    protected JsonContextPoolBase(Size? maxContextSizeToKeep, IIronLogger logger)
        : this(logger)
    {
    ...
    protected JsonContextPoolBase(Size? maxContextSizeToKeep, long? maxNumberOfContextsToKeepInGlobalStack, IIronLogger logger)
    ...
    public IDisposable AllocateOperationContext(out T context)
    {
        ...
        while (_perCoreCache.TryPull(out var pulled))
        {
            if (pulled!.InUse.Raise() == false)
                continue;
