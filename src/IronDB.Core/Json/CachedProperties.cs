...
public sealed class CachedProperties
{
    ...
    public sealed class PropertyName : IComparable<PropertyName>
    {
        ...
        public PropertyName(int hash, LazyStringValue comparer, int globalSortOrder, int propertyId)
        {
            ...
            IsVectorProperty = Comparer.AsReadOnlySpan().SequenceEqual(Constants.Naming.VectorPropertyNameAsSpan);
        }
    ...
    public const int CachedPropertiesSize = 512;

    private CachedSort[]? _cachedSorts;

    private readonly FastList<PropertyName> _docPropNames = [];
    private readonly SortedDictionary<PropertyName, object> _propertiesSortOrder = [];
    ...
    public void Sort(FastList<BlittableJsonDocumentBuilder.PropertyTag> properties)
    {
        ...
        CachedSort? cachedSort = _cachedSorts?[index];
