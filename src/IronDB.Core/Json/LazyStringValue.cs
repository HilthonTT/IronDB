...
{
    public static readonly LazyStringValueComparer Instance = new();
...
{
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public readonly bool Equals(LazyStringValue? x, LazyStringValue? y)
    {
        if (x == y)
        {
            return true;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public readonly int GetHashCode(LazyStringValue obj)
    {
        unsafe
        {
...
#endif
{
    ...
    public int CompareTo(string? other)
    {
        ...
        var buffer = GetLazyStringTempComparisonBuffer(other?.Length ?? 0);
        fixed (char* pOther = other)
        fixed (byte* pBuffer = buffer)
        {
            var tmpSize = Encodings.Utf8.GetBytes(pOther, other?.Length ?? 0, pBuffer, buffer.Length);
    ...
    public static implicit operator byte[](LazyStringValue self)
    {
        ...
        return Convert.FromBase64String(valueAsString ?? string.Empty);
    }
    ...
    {
        return (string?)this ?? string.Empty; // invoke the implicit string conversion
    }
