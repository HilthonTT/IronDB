...
public sealed unsafe class ArenaMemoryAllocator : IDisposable
{
    ...
    private static readonly int? SingleAllocationSizeLimit = PlatformDetails.Is32Bits ? 8 * Constants.Size.Megabyte : null;
...
#pragma warning restore 649
    ...
    private void GrowArena(int requestedSize)
    {
        ...
        if (_olderBuffers is null)
        {
            _olderBuffers = [];
        }

        if (_allocatingThread is not null)
        {
    ...
    public void ResetArena()
    {
        ...
        if (_olderBuffers is null)
        {
            ...
            if (_allocated / 2 > _used && _allocated > _initialSize * 2)
            {
                ...
                if (_ptrStart is not null && _allocatingThread is not null)
                {
        ...
        if (SingleAllocationSizeLimit is not null && _allocated > SingleAllocationSizeLimit.Value)
        {
