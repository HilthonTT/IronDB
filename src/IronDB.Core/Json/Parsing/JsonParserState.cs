...
public sealed unsafe class JsonParserState
{
    ...
    public readonly FastList<int> EscapePositions = [];
    ...
    public static void FindEscapedPositionsAndEscapeControls(
        FastList<int> buffer, 
        byte* str,
        ref int len, 
        int previousComputedMaxSize)
    {
        var originalLen = len;
        buffer.Clear();
    ...
    internal int FillVector<T>(Span<T> vector) where T : unmanaged
    {
        ...
        for (int i = 0; i < vector.Length; i++)
        {
            ...
            if (item.Item1 == JsonParserToken.Float)
            {
                ...
                throw new InvalidOperationException($"The type {typeof(T).Name} is not supported when we have buffered floating point values.");
            }
            else
            {
