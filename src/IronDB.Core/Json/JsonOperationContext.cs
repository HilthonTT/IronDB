...
#pragma warning restore CS0649
    ...
    {
        if (_allocateStringValues is not null && _numberOfAllocatedStringsValues < _allocateStringValues.Count)
        {
        ...
        if (_allocateStringValues is not null && _numberOfAllocatedStringsValues < _maxNumberOfAllocatedStringValues)
        {
    ...
    public AllocatedMemoryData GetLongLivedMemory(int requestedSize)
    {
        ...
        if (requestedSize <= 0)
        {
            throw new ArgumentException(null, nameof(requestedSize));
        }    
#endif
        //we should use JsonOperationContext in single thread

        ObjectDisposedException.ThrowIf(_arenaAllocatorForLongLivedValues is null, "Could not allocated long lived memory, because the context is after Reset() but before Renew(). Is it possible that you have tried to use the context AFTER it was returned to the context pool?");
    ...
    public unsafe LazyStringValue GetLazyStringRaw(byte* ptr, int size, bool longLived = false)
    {
        ...
        LazyStringValue result = !longLived ? AllocateStringValue(null, address, size) : new LazyStringValue(null, address, size, this);
        result.AllocatedMemoryData = memory;
        return result;
    }
    ...
    public readonly struct AvoidOverAllocationScope(JsonOperationContext parent) : IDisposable
    {
        private readonly JsonOperationContext _parent = parent;
    ...
    private readonly IntReference _threadId = new() { Value = 0 };
