...
public static unsafe class Memory
{
    ...
    private static ReadOnlySpan<byte> LoadMaskTable =>
    [
        0xFF, 0xFF, 0xFF, 0xFF,
        0xFF, 0xFF, 0xFF, 0xFF,
        0xFF, 0xFF, 0xFF, 0xFF,
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public unsafe static T Read<T>(byte* bptr)
    {
        return Unsafe.Read<T>(bptr);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe bool IsEqualConstant(ReadOnlySpan<byte> constant, byte* bptr)
    {
#if NET7_0_OR_GREATER
        ...
        if (AdvInstructionSet.X86.IsSupportedAvx256)
        {
            return IsEqualConstantAvx256(ref constantRef, bptr, constant.Length);
        }

        return IsEqualConstantVector128(ref constantRef, bptr, constant.Length);
#endif
        throw new NotSupportedException($"{nameof(IsEqualConstant)} is not supported in frameworks lesser than 7.0");
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe bool IsEqualConstant(byte* constant, int size, byte* bptr)
    {
#if NET7_0_OR_GREATER
        ...
        if (AdvInstructionSet.X86.IsSupportedAvx256)
        {
            return IsEqualConstantAvx256(ref constantRef, bptr, size);
        }

        return IsEqualConstantVector128(ref constantRef, bptr, size);
#else
            throw new NotSupportedException($"{nameof(IsEqualConstant)} is not supported in frameworks lesser than 7.0");
#endif
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe bool IsEqualConstant(ReadOnlySpan<byte> constant, byte* bptr, int size)
    {
#if NET7_0_OR_GREATER
        // PERF: The intended use of this method is when the ReadOnlySpan<byte> is a constant, for anything
        // else the execution cost of this method will be horrible. The reason why is that when we are 
