...
internal static class TimeoutManager
{
    ...
    {
        if (bool.TryParse(Environment.GetEnvironmentVariable("IRON_TIMEOUTMANAGER_USE_TASK_DELAY"), out var useTaskDelay))
        ...
        public Task NextTask
        {
            get
            {
                while (true)
                {
                    ...
                    if (Interlocked.CompareExchange(ref _nextTimeout, tcs, null) is null)
                    {
                        return tcs.Task;
                    }
                }
