...
internal sealed class LowMemoryNotification
{
    ...
    private readonly IIronLogger _logger;

    private readonly ConcurrentSet<WeakReference<ILowMemoryHandler>> _lowMemoryHandlers = [];
        ...
        try
        {
            ...
            try
            {
                ...
                if ((logInInfo || _logger.IsDebugEnabled) && now - _lastLoggedLowMemory > _logLowMemoryInterval)
                {
                    ...
                    string message = 
                        $"Running {lowMemoryHandlersCount} low memory handlers with severity: {lowMemorySeverity}. {MemoryUtils.GetExtendedMemoryInfo(memoryInfo, _lowMemoryMonitor?.GetDirtyMemoryState() ?? new DirtyMemoryState())}";
    ...
    private void ClearInactiveHandlers()
    {
        ...
        foreach (var lowMemoryHandler in _lowMemoryHandlers)
        {
            if (!lowMemoryHandler.TryGetTarget(out _))
            {
    ...
    public static readonly LowMemoryNotification Instance = new();
    ...
    private readonly ManualResetEvent _simulatedLowMemory = new(false);
    private readonly ManualResetEvent _shutdownRequested = new(false);
    private readonly List<WeakReference<ILowMemoryHandler>> _inactiveHandlers = new(2048);
    ...
    {
        _logger = IronLogManager.Instance.GetLogger<LowMemoryNotification>()!;
    }
                ...
                try
                {
                    ...
                    switch (result)
                    {
                        case WaitHandle.WaitTimeout:
                            if (_lowMemoryMonitor is not null)
                            {
    ...
    private void AddLowMemEvent(LowMemReason reason, MemoryInfoResult memoryInfo, long totalUnmanaged)
    {
        var lowMemEventDetails = new LowMemEventDetails
        {
            ...
            TotalScratchDirty = _lowMemoryMonitor?.GetDirtyMemoryState().TotalDirty.GetValue(SizeUnit.Bytes) ?? 0,
