...
public readonly struct Sorter<T, TSorter>(in TSorter sorter)
    where TSorter : struct, IComparer<T>
{
    private readonly TSorter _sorter = sorter;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static void Swap(Span<T> keys, int x, int y)
    {
        ref T kx = ref keys[x];
        ref T ky = ref keys[y];
    ...
    private static int PickPivotAndPartitionUnlikely(Span<T> keys, int lo, int hi, int middle)
    {
        Swap(keys, middle, hi - 1);
    ...
    private void InsertionSort(Span<T> keys, int lo, int hi)
    {
        ...
        for (int i = lo; i < hi; i++)
        {
            ...
            while (j >= lo && (t is null || _sorter.Compare(t, keys[j]) < 0))
            {
                keys[j + 1] = keys[j];
                j--;
            }
    ...
    where TSorter : struct, IComparer<T>
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static void Swap(Span<T> keys, Span<V> values, int x, int y)
    {
        T aux = keys[x];
        V vAux = values[x];
    ...
    private static int PickPivotAndPartitionUnlikely(Span<T> keys, Span<V> values, int lo, int hi, int middle)
    {
        Contract.Requires(keys != Span<T>.Empty);
        Contract.Requires(lo >= 0);
        Contract.Requires(hi > lo);
