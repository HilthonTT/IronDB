...
{
    public static readonly TimeValue Zero = new()
    {
        Value = 0
    };

    public static readonly TimeValue MaxValue = new()
    {
        Value = int.MaxValue
    };

    public static readonly TimeValue MinValue = new()
    {
        Value = int.MinValue
    };
    ...
    public double TimesInInterval(DateTime from, DateTime to)
    {
        ...
        return Unit switch
        {
            TimeValueUnit.Month => (to.TotalMonths() - from.TotalMonths()) / (double)Value,
            TimeValueUnit.Second => to.Subtract(from).TotalSeconds / Value,
            TimeValueUnit.None => 0,
            _ => throw new ArgumentException(),
        };
    }

    public readonly bool IsMultiple(TimeValue multiple)
    {
    ...
    private static void Append(StringBuilder builder, int value, string singular)
    {
        if (value <= 0)
        {
            return;
    ...
    private readonly void AssertSeconds()
    {
    ...
    public readonly int Compare(TimeValue other)
    {
        ...
        var (UpperBound, LowerBound) = GetBoundsInSeconds(this);
        var otherBounds = GetBoundsInSeconds(other);

        if (otherBounds.UpperBound < LowerBound)
        {
            return 1;
        }

        if (otherBounds.LowerBound > UpperBound)
        {
            return -1;
        }
