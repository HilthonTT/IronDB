...
public sealed class TcpConnectionMonitor
{
    ...
    private static readonly ILogger Logger = Log.ForContext<TcpConnectionMonitor>();
    ...
    private void AnalyzeConnection(ConnectionData connectionData)
    {
        ...
        {
            Logger.Information("# {connection} is faulted", connection);
            return;
        }
    ...
    private static void CheckMissingSendCallback(ConnectionData connectionData, IMonitoredTcpConnection connection)
    {
        // snapshot all data?
    ...
    private static void CheckPendingSend(IMonitoredTcpConnection connection)
    {
        ...
        {
            Logger.Information("# {connection} {pendingSendKiloBytes}kb pending send", connection, pendingSendBytes / 1024);
        }
    }
    ...
    private static void CheckPendingReceived(IMonitoredTcpConnection connection)
    {
        ...
        {
            Logger.Information("# {connection} {pendingReceivedKiloBytes}kb are not dispatched", connection,
